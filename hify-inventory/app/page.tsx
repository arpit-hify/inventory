  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end modal-backdrop fade-in">
      <div className="slide-up" style={S.sheetWrap}>
        ...
        {pi.qr_code && (
          <div style={{marginBottom:16}}>
            <div style={{background:'white',borderRadius:12,padding:12,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
              <img src={pi.qr_code} alt="QR Code" style={{width:180,height:180}}/>
            </div>
            <div style={{display:'flex',gap:8}}>
              <a href={pi.qr_code} download={`${pi.label}-qr.png`}
                style={{flex:1,height:38,borderRadius:10,background:'rgba(167,139,250,0.12)',color:'var(--hify-purple)',display:'flex',alignItems:'center',justifyContent:'center',gap:6,fontSize:13,fontWeight:600,textDecoration:'none'}}>
                <I.Download/> Download QR
              </a>
              <button onClick={()=>onGenerateQR(pi)}
                style={{height:38,padding:'0 14px',borderRadius:10,background:'var(--hify-surface2)',color:'var(--hify-muted)',display:'flex',alignItems:'center',gap:5,fontSize:12,fontWeight:600,border:'none',cursor:'pointer'}}>
